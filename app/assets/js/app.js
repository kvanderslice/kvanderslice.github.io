!(function ($) {
  ...
  app = {
    ...
    config: {
      ...
      debug: !!window.location.href.match(/(localhost)/g)
    },
  ...
  //   // appendArrows: $('.slick-arrow-container') ,
  //   nextArrow: '<a class="icon fa fa-angle-right slick-arrow--next"></a>',
  //   prevArrow: '<a class="icon fa fa-angle-left  slick-arrow--prev"></a>',
  //   responsive: [
  //     {
  //       breakpoint: 768,
  //       settings: {
  ...
  app.waypoint = function (element, offset, callback) {
    ...
    window.addEventListener('scroll', function (event) {
      ...
      var window_offset_top = document.documentElement.scrollTop || document.body.scrollTop
  ...
  app.scroller = {
    ...
    update: function () {
      ...
      var y = _this.last_known_y

      _this.ticking = false

      direction.down = !((_this.previous_y > y))
      direction.up = !((_this.previous_y < y))
      ...
      if (_this.debug) {
        console.log('%cEVENT', 'color:' + app.console.color.event, ' requestAnimationFrame', {
          'up': direction.up,
          'down': direction.down,
          'pageY': y
        })
   ...
   */
  function isIE () {
    var undef; var rv = -1 // Return value assumes failure.
